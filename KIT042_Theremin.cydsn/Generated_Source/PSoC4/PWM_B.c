...
void PWM_B_Init(void)
{
    ...
    /* Set values from customizer to CTRL */
        ...
        PWM_B_CONTROL_REG =
        (((uint32)(PWM_B_QUAD_ENCODING_MODES     << PWM_B_QUAD_MODE_SHIFT))       |
         ((uint32)(PWM_B_CONFIG                  << PWM_B_MODE_SHIFT)));
    #endif  /* (PWM_B__QUAD == PWM_B_CONFIG) */

    #if (PWM_B__PWM_SEL == PWM_B_CONFIG)
        PWM_B_CONTROL_REG =
        (((uint32)(PWM_B_PWM_STOP_EVENT          << PWM_B_PWM_STOP_KILL_SHIFT))    |
         ((uint32)(PWM_B_PWM_OUT_INVERT          << PWM_B_INV_OUT_SHIFT))         |
         ((uint32)(PWM_B_PWM_OUT_N_INVERT        << PWM_B_INV_COMPL_OUT_SHIFT))     |
         ((uint32)(PWM_B_PWM_MODE                << PWM_B_MODE_SHIFT)));

        #if (PWM_B__PWM_PR == PWM_B_PWM_MODE)
            PWM_B_CONTROL_REG |=
            ((uint32)(PWM_B_PWM_RUN_MODE         << PWM_B_ONESHOT_SHIFT));

            PWM_B_WriteCounter(PWM_B_PWM_PR_INIT_VALUE);
        #else
            PWM_B_CONTROL_REG |=
            (((uint32)(PWM_B_PWM_ALIGN           << PWM_B_UPDOWN_SHIFT))          |
             ((uint32)(PWM_B_PWM_KILL_EVENT      << PWM_B_PWM_SYNC_KILL_SHIFT)));
        #endif  /* (PWM_B__PWM_PR == PWM_B_PWM_MODE) */

        #if (PWM_B__PWM_DT == PWM_B_PWM_MODE)
            PWM_B_CONTROL_REG |=
            ((uint32)(PWM_B_PWM_DEAD_TIME_CYCLE  << PWM_B_PRESCALER_SHIFT));
        #endif  /* (PWM_B__PWM_DT == PWM_B_PWM_MODE) */

        #if (PWM_B__PWM == PWM_B_PWM_MODE)
            PWM_B_CONTROL_REG |=
            ((uint32)PWM_B_PWM_PRESCALER         << PWM_B_PRESCALER_SHIFT);
        #endif  /* (PWM_B__PWM == PWM_B_PWM_MODE) */
    #endif  /* (PWM_B__PWM_SEL == PWM_B_CONFIG) */

    #if (PWM_B__TIMER == PWM_B_CONFIG)
        PWM_B_CONTROL_REG =
        (((uint32)(PWM_B_TC_PRESCALER            << PWM_B_PRESCALER_SHIFT))   |
         ((uint32)(PWM_B_TC_COUNTER_MODE         << PWM_B_UPDOWN_SHIFT))      |
         ((uint32)(PWM_B_TC_RUN_MODE             << PWM_B_ONESHOT_SHIFT))     |
         ((uint32)(PWM_B_TC_COMP_CAP_MODE        << PWM_B_MODE_SHIFT)));
    #endif  /* (PWM_B__TIMER == PWM_B_CONFIG) */

    /* Set values from customizer to CTRL1 */
    #if (PWM_B__QUAD == PWM_B_CONFIG)
        PWM_B_TRIG_CONTROL1_REG  =
        (((uint32)(PWM_B_QUAD_PHIA_SIGNAL_MODE   << PWM_B_COUNT_SHIFT))       |
         ((uint32)(PWM_B_QUAD_INDEX_SIGNAL_MODE  << PWM_B_RELOAD_SHIFT))      |
         ((uint32)(PWM_B_QUAD_STOP_SIGNAL_MODE   << PWM_B_STOP_SHIFT))        |
         ((uint32)(PWM_B_QUAD_PHIB_SIGNAL_MODE   << PWM_B_START_SHIFT)));
    #endif  /* (PWM_B__QUAD == PWM_B_CONFIG) */

    #if (PWM_B__PWM_SEL == PWM_B_CONFIG)
        PWM_B_TRIG_CONTROL1_REG  =
        (((uint32)(PWM_B_PWM_SWITCH_SIGNAL_MODE  << PWM_B_CAPTURE_SHIFT))     |
         ((uint32)(PWM_B_PWM_COUNT_SIGNAL_MODE   << PWM_B_COUNT_SHIFT))       |
         ((uint32)(PWM_B_PWM_RELOAD_SIGNAL_MODE  << PWM_B_RELOAD_SHIFT))      |
         ((uint32)(PWM_B_PWM_STOP_SIGNAL_MODE    << PWM_B_STOP_SHIFT))        |
         ((uint32)(PWM_B_PWM_START_SIGNAL_MODE   << PWM_B_START_SHIFT)));
    #endif  /* (PWM_B__PWM_SEL == PWM_B_CONFIG) */

    #if (PWM_B__TIMER == PWM_B_CONFIG)
        PWM_B_TRIG_CONTROL1_REG  =
        (((uint32)(PWM_B_TC_CAPTURE_SIGNAL_MODE  << PWM_B_CAPTURE_SHIFT))     |
         ((uint32)(PWM_B_TC_COUNT_SIGNAL_MODE    << PWM_B_COUNT_SHIFT))       |
         ((uint32)(PWM_B_TC_RELOAD_SIGNAL_MODE   << PWM_B_RELOAD_SHIFT))      |
         ((uint32)(PWM_B_TC_STOP_SIGNAL_MODE     << PWM_B_STOP_SHIFT))        |
         ((uint32)(PWM_B_TC_START_SIGNAL_MODE    << PWM_B_START_SHIFT)));
    #endif  /* (PWM_B__TIMER == PWM_B_CONFIG) */
...
*
* Summary:
*  Initialize the PWM_B with default customizer
...
*  PWM_B operates in continuous mode.
*
* Parameters:
*  oneShotEnable
*   Values:
