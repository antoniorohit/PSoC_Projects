...
extern const uint8 CYCODE CapSense_CSD_widgetNumber[];
...
uint16 CapSense_CSD_sensorSignal[CapSense_CSD_TOTAL_SENSOR_COUNT] = {0u};
uint8 CapSense_CSD_sensorOnMask[(((CapSense_CSD_TOTAL_SENSOR_COUNT - 1u) / 8u) + 1u)] = {0u};
...
/* Helps while centroid calulation */
    ...
    static uint16 CapSense_CSD_centroid[3];
...
*  Resets to zero CapSense_CSD_sensorBaselineLow[senesor] and 
 ...
 void CapSense_CSD_UpdateSensorBaseline(uint8 sensor)
{
    ...
    {
		CapSense_CSD_lowBaselineResetCnt[sensor] = 0;	
 ...
 void CapSense_CSD_UpdateEnabledBaselines(void)
{
    ...
    for(i = 0; i < CapSense_CSD_TOTAL_SENSOR_COUNT; i++)
    {
        pos = (i >> 3u);
        enMask = 0x01u << (i & 0x07u);
        if((CapSense_CSD_sensorEnableMask[pos] & enMask) != 0u)
...
uint8 CapSense_CSD_CheckIsSensorActive(uint8 sensor)
{
    ...
    /* Was on */
    if (CapSense_CSD_sensorOnMask[pos] & onMask)
    {
        /* Hysteresis minus */
    ...
    return (CapSense_CSD_sensorOnMask[pos] & onMask) ? 1u : 0u;
}
...
void CapSense_CSD_DisableWidget(uint8 widget)
{
    ...
    *******************************************************************************/
                                        ...
                                        uint8 count, uint16 resolution, uint16 noiseThreshold)
    {
        ...
        /* Si+1 - Si-1 */
        numerator = (int32) CapSense_CSD_centroid[CapSense_CSD_POS_NEXT] - 
                    (int32) CapSense_CSD_centroid[CapSense_CSD_POS_PREV];
        ...
        position = ((uint8) HI16(denominator + CapSense_CSD_CENTROID_ROUND_VALUE));
                    ...
                    if (0u != (posFiltersMask & CapSense_CSD_ANY_POS_FILTER))
                    {
                        ...
                        else
                        {
                            /* Do filtering */
                                ...
                                if (0u != (posFiltersMask & CapSense_CSD_IIR4_FILTER))
                                {
                                    posX = (uint8) CapSense_CSD_IIR4Filter(posX, 
                                                                            CapSense_CSD_posFiltersData[posXIndex]);
                                    CapSense_CSD_posFiltersData[posXIndex] = posX;
                                    
                                    posY = (uint8) CapSense_CSD_IIR4Filter(posY, 
                                                                            CapSense_CSD_posFiltersData[posYIndex]);
                                    CapSense_CSD_posFiltersData[posYIndex] = posY;
                                }
    ...
    *******************************************************************************/
    uint8 CapSense_CSD_GetMatrixButtonPos(uint8 widget, uint8* pos)
    {
        ...
        if (CapSense_CSD_CheckIsWidgetActive(widget))
        {
