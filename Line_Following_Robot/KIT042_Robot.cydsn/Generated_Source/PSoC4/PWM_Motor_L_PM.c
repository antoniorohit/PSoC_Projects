...
static PWM_Motor_L_BACKUP_STRUCT PWM_Motor_L_backup;
...
{
    if(0u != (PWM_Motor_L_BLOCK_CONTROL_REG & PWM_Motor_L_MASK))
    {
        PWM_Motor_L_backup.enableState = 1u;
    }
    else
    {
        PWM_Motor_L_backup.enableState = 0u;
    }
...
void PWM_Motor_L_Wakeup(void)
{
    ...
    if(0u != PWM_Motor_L_backup.enableState)
    {
        PWM_Motor_L_Enable();
    }
}
